...
func buildTemplateFuncs(t i18n.Strings) template.FuncMap {
	return template.FuncMap{
		// i18n accessor — T "FieldName"
		"T": func(key string) string {
			switch key {
			case "TitleSuffix":         return t.TitleSuffix
			case "LabelServers":        return t.LabelServers
			case "LabelTools":          return t.LabelTools
			case "LabelFlagged":        return t.LabelFlagged
			case "NoLaunchBannerTitle": return t.NoLaunchBannerTitle
			case "NoLaunchBannerBody":  return t.NoLaunchBannerBody
			case "NoticeBannerTitle":   return t.NoticeBannerTitle
			case "NoticeBannerBody":    return t.NoticeBannerBody
			case "ConfigPanelToggle":   return t.ConfigPanelToggle
			case "SectionCommand":      return t.SectionCommand
			case "SectionAllowedDirs":  return t.SectionAllowedDirs
			case "SectionEnvVars":      return t.SectionEnvVars
			case "ColToolName":         return t.ColToolName
			case "ColDescription":      return t.ColDescription
			case "ColBadges":           return t.ColBadges
			case "EmptyTools":          return t.EmptyTools
			case "ErrorPrefix":         return t.ErrorPrefix
			case "NoLaunchLabel":       return t.NoLaunchLabel
			case "ToolListToggle":      return t.ToolListToggle
			}
			return key
		},
...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .path-list li::before { content: "📁"; font-size: 11px; flex-shrink: 0; }
...
<div class="container">
  ...
  <div class="header">
    <h1>🔍 MCP Inspect Report{{if .NoLaunch}} <span style="font-size:14px;font-weight:400;color:#6c757d;">{{T "TitleSuffix"}}</span>{{end}}</h1>
  ...
  <div class="summary">
    <div class="summary-card">
      <div class="num">{{len .Servers}}</div>
      <div class="label">{{T "LabelServers"}}</div>
    </div>
    {{if not .NoLaunch}}
    <div class="summary-card">
      <div class="num">{{.TotalTools}}</div>
      <div class="label">{{T "LabelTools"}}</div>
    </div>
    <div class="summary-card">
      <div class="num">{{.TotalWarnings}}</div>
      <div class="label">{{T "LabelFlagged"}}</div>
    </div>
    {{range $w, $n := .WarnBreakdown}}
    <div class="summary-card">
      <div class="num">{{$n}}</div>
      <div class="label">{{breakdownLabel $w}}</div>
    </div>
    {{end}}
    {{end}}
  </div>
  ...
  {{if .NoLaunch}}
  <div class="nolaunch-banner">
    📋 <strong>{{T "NoLaunchBannerTitle"}}</strong> {{T "NoLaunchBannerBody"}}
  </div>
  {{else}}
  <div class="notice">
    <strong>{{T "NoticeBannerTitle"}}</strong> {{T "NoticeBannerBody"}}
  </div>
  {{end}}
  ...
  <div class="server">
    ...
    <div class="server-error">
      ...
      {{with errorHint .}}<div class="error-hint">💡 {{.}}</div>{{end}}
    </div>
      ...
      <table>
        ...
        <tbody>
          ...
          <tr>
            ...
            <td class="tool-warnings">
              {{range .Warnings}}<span class="badge {{warnClass .}}">{{warnLabel .}}</span>{{end}}
            </td>
          </tr>
          {{end}}
        </tbody>
  ...
  <div class="footer">
    Generated by <a href="https://github.com/masuda-masuo/mcp-inspect">mcp-inspect</a>
  </div>
</div>
</body>
</html>`
